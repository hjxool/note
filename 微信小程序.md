...
- 获取节点使用`createSelectorQuery()`
  ...
  - 自定义组件或当前页面使用`this.createSelectorQuery()`
    ...
    - 注意！这有个坑，==在组件内==，如果使用`wx/uni`调用`this.createSelectorQuery`是==无法获取到节点信息==的！
    ...
    // uniapp
    const 组件实例 = getCurrentInstance().proxy
    uni.createSelectorQuery().in(组件实例)
    ```
     ...
     - 获取整个集合数据/==分页==查询
       ...
       async function get_data(pageNum, pageSize){
           ...
           return await testdoc
               ...
               .get()
           // 分页条件查询查询 返回总数
           // 可以用await将Promise链式中任意步骤取出保存 以便后面使用
           let d = await testdoc.where(condition)
...
- 示例
  ...
  - 正则对象`db.RegExp`的`options`参数
    - `i`：大小写不敏感
    - `m`：跨行匹配
      - 让开始匹配符 `^` 或结束匹配符` $ `时除了匹配字符串的开头和结尾外，还匹配行的开头和结尾
  ...
  db.collection('todos').where(_.or([
      {
      	user_name: db.RegExp({
             regexp: keyword, // 传入的关键字
             options: 'i',
        	})
      },
      { // 查询关键字符合user_name或pet_name中任意位置 返回对应记录
          pet_name: db.RegExp({
              regexp: keyword,
              options: 'i',
          })
...
1. 调用`wx.chooseMedia`选择图片等媒体文件
   ...
   wx.chooseMedia({
     ...
     success(res) {
       // 成功回调 返回文件信息数组
       console.log(res.tempFiles[0].tempFilePath)//本地文件临时路径 有文件格式
...
2. `wx.cloud.uploadFile`只能单个上传，封装使其可以批量上传
   ...
   - 注：上传同名、同路径文件会替换对应文件
   - `fileID`和文件名不需要文件后缀也能显示，如文件名`1`，`fileID`为`cloud://.../room_photo/1`，依然可以显示图片
...
- `root-portal`
  ...
  - 适用于中间显示的小弹窗
    ...
    - 需要封装在弹窗组件里
    - 样式全都要自己写
    - 相当于`position:fixed;`容器

  - 示例

    - 父页面wxml引入`popup`自定义组件

      ```html
      <view>
      	<popup show="{{show}}" catch:close_pop="close_pop">
              <!--传入片段 样式写在父页面-->
    ...
    - `popup`组件wxml
      ...
      <root-portal wx:if="{{show}}">
        <view class="popup animation {{pop_hide?'pop_hide':''}}"
              bindtap="close">
          <!--默认插槽 由引用组件时传入的片段决定显示内容-->
          <slot></slot>
        </view>
      </root-portal>
...
- `page-container`
  ...
  - 适用于大弹窗，近似"假页面"，可以选择从中间、四周弹出，并带有过渡动画选项
  ...
  - 示例
    ...
    - wxml
      ...
      ```html
      <!-- 设置按钮 -->
      <button class="btn" bindtap="popup" data-position="bottom">
          底部弹出
      </button>
...
- 小程序中提供的`scroll-view`容器就包含下拉刷新的功能和样式
...
- 示例
  ...
  - wxml
    ...
    - `refresher-triggered`下拉刷新是否被触发(==复位==)
    - `bind:refresherrefresh`下拉刷新被触发**事件**
    - `bind:refresherrestore`下拉刷新被复位**事件**
...
2. 标签上要加`<text decode="{{true}}">`

3. 设置样式

   ```css
   text{
       word-break: break-all; /* 文字过长自动断开 */
   }
   ```
