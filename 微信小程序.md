...
- 父页面|组件向子组件通信传值
  ...
  - 获取组件实例
    ...
    - 自定义组件被 `selectComponent` 调用时的返回值

      - 自定义组件实例必须使用内置`wx://component-export`
...
- 组件引入公共内容`behaviors`
  ...
  - 注：`Behavior`中不是所有配置项都有！比如`Component({options:{...}})`就没有！
    ...
    // my-component.js
    import option from 'my-behavior'
    Component({
    ...
    Component({
        ...
        observers: {
            ...
            'numberA, numberB': function(numberA, numberB){
                ...
                this.setData({
                    sum: this.data.numberA + this.data.numberB
                })
            }
        }
    })
...
2. `wx.cloud.uploadFile`只能单个上传，封装使其可以批量上传
   ...
   async function uploadFile(list, cloud_path){
       ...
       let res = await Promise.all(result).catch(err => 'err')
       // 全部上传结束(无论成功与否)关闭遮罩
       if(res !== 'err'){
          // 如果是数组说明全部上传成功
       }
