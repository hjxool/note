...
4. 得到集合的引用后即可操作文档记录
   ...
   - 更新数据
     ...
     - `update`局部更新一个或多个文档
       ...
       - `set`的用处在于更新一个值为一个对象。如下例是更新`cusKey4.cusKey5`字段值为`'ttt'`，而不是把`cusKey4`字段值更新为`{cusKey5: 'ttt'}`对象
       ...
       - 如果要整个==替换==`cusKey4`对象，就需要使用`set`指令
...
- Tips
  - 同一个用户在同一个小程序中的`_openid`是不变的，且唯一，用于识别用户身份