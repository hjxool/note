...
​    10、在“mounted”中操作节点，因为只有这时才将真实节点放入页面了

​    11、vue对**数组**中的元素不会进行数据代理，只对**数组元素以外**的所有变量类型，进行数据代理。所以**修改**数组**元素**不会引起vue**更新模板**，只有通过Array中**改变原数组**的方法，才会引起虚拟DOM更新；注意！数组中的元素，哪怕是对象也是没有响应式的。(我之前没有遇到这个问题是因为没有修改过单个数组元素，都是整体替换，而数组本身是有数据代理的)

​    12、数据**劫持**和数据**代理**是一个意思，都是用**defineProperty()**

​    13、解决页面闪动(加载速度问题)，可以使用”v-cloak“——这是一种特殊属性，在页面加载完后，vue会删除这一属性，搭配css，可以将写了”v-cloak属性“的地方都隐藏起来，直到vue正式接管”容器“后再删除显示元素标签

​    14、要定义**全局**属性、指令等，使用“Vue.method”等函数注册全局

​    15、组件渲染速度就是比较慢，在vue实例对象里初始调用节点是可以的，但是组件中不行

​    16、**只有data或者set()中的属性变量才有响应式！**

​    17、函数并不是必须用`**()**`，而是在只需要返回结果时才用`**()**`

​    18、数据的根在哪个组件就在哪操作数据！

------

{{ }}插值表达式，将数据和页面关联；el挂载点，使用方法类同css选择器；
...
两个方式的区别在于v-text是将标签内的内容全部提换；另一个是只替换指定位置
...
​	中使用“this”，本身的指向应该是windows，但是这里vue帮你做了一件事，就是把“this”的指向改成“vue实例”，所以在这类vue给定的特殊函数中，可以用this
...
watch可以用于限制输入，因为输入的时候先进来的是newValue，如果不做任何操作才会显示在界面上，但是如果做条件判断再给监听的属性重新赋值，看起来就会像输入框限制用户输入一样
...
watch其实是根据字符串查找属性，所以“$watch”中第一个参数也是“字符串形式”
...
当data中数据层层嵌套时，用简写的形式——直接写属性名是不行的，因为“key：value”的key“不合规”，所以要用完整形式书写——字符串形式(当中嵌套调用的“.”是不能省略的哦)
...
想要监听嵌套对象本身是否发生改变，需要使用“deep”选项，因为监听对象，只是监听它所引用的地址，其内容本身的变化并不会触发监听
...
​            2、created阶段，data中的变量和methods中的方法都已经创建好了，但是在生命周期函数中为“data变量”赋值时，data中的数据是不能为空的，要么随便给一个初始值，要么用“Number、String”等类限制传入数据类型
...
- 通过[$emit]()及[$on]()触发自定义事件：`<component @自定义事件="fatherFun" />`、`组件内：this.$emit('自定义事件', 参数)`
...
- 通过[$parent]()获取父级方法：`this.$parent.fatherFunction(参数)`
