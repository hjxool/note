...
- 不同函数的this区别如下：
  ...
  - 箭头函数：**定义时**被决定
    ...
    - 箭头函数的this是==父级==**函数**的this==，**没有则指向window**，==**对象是没有this的！只有函数才有this！**
...
- [slice(start,end)]()：截取==包括==开始到==不包含==结束的数组元素并作为一个==新数组==返回。
...
- `filter( (当前元素的值) => { return 条件} )`：过滤器，返回指定条件的元素组成的==数组==，不会改变原数组；“当前值”是Arry中每一个元素，因为filter其实执行了一次遍历，将Arry中每一个元素拿出来跟return中的判断条件进行比对，再将符合条件的元素合成一个新数组
...
- [indexOf('检索值'，'起始位置')]()：检索字符串中是否有**指定内容**，有则返回**位置索引**，没有则返回**-1**；

  - 注意，检索“空字符串 ' ' ”时，返回的值是“0”，因为所有字符串在开头都拥有空字符串(**也可以用于数组**)，==但不能使用正则去匹配==

  - ==区分大小写==

- [match]()：`string.match(正则表达式)`

  - 匹配内容**不包括**==括号内子匹配==内容，只有==整体匹配==结果，以**数组形式**返回
  - `\`是==转义字符==，用`"我说：\"今天天气不错\" "`输出出来就是`我说："今天天气不错"`

- [matchAll]()：`string.matchAll(正则表达式)`，ES11新增方法
...
- ==小技巧==：当一个函数同时执行两个不相干==动作==时，会卡顿，比如：骑自行车需要脚提供速度和手提供方向，如果将这两个动作放在一起，整个过程就会变得卡顿！
