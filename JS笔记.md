...
- ※`Math.random()`：生成==**包括**0~**不包括**1==的随机数
  ...
  - 生成==min~max==的随机数：`Math.round( Math.random() * ( max - min ) + min )`（思路是：0~1乘一个数扩大范围，再加一个数确保最小值，但是要保证不超限，所以乘的数等于差值）
...
- ```js
  try{
      ...
      // try中有return时 先执行try中return前部分 然后执行finally 再执行try中return
  }
