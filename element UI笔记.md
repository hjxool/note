...
- 使用==非element提供的**事件**==，如：`@keyupo`等，需要用==.native==修饰符
...
- [el-time-picker]()：精确到**秒**。
  ...
  - 也可以传入==字符串==(内部应该是调用了new Date)，形如==2017-10-1 10:00:00==，==必须有==年月日，不写时分秒会默认八点
  
- [日期]()选择器==接收参数==可以是==字符串==和==Date==对象，字符串只要不是连在一起的日期，都能识别
  - 字符串可以不写时分秒
  - 但是==onchange事件==获得的参数都是==Date对象==，不是==字符串！==
  - 输出结果是==Date对象==
    - ==开始到结束日期==，返回结果是开始日期的00点Date对象到==结束日期的00点Date对象==，组成的==数组==
...
- [page-size]()：一页最大显示数量。必不可少的属性，少了的话会出现分页错
- ==插槽==
  - 在==layout==属性中进行排序，`<el-pagination layout="slot, total,..."/>`，`slot`在layout中的==位置==决定`<el-pagination> <span>插槽内容</span> </el-pagination>`插槽内容在组件中显示的位置
  - 可以有多个slot，`layout="slot, total, slot,..."`，但无法指定每个插槽内容分配到哪里，而是将全部插槽内容复制多份到不同slot位置
...
- `size`属性要设置在`<el-button>`标签上，设置在`<el-dropdown>`上无效，因为本质是`<el-button>`撑大的